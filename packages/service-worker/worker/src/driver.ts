...
export class Driver implements Debuggable, UpdateSource {
  ...
  private async handleFetch(event: FetchEvent): Promise<Response> {
    ...
    try {
      ...
      if (err.isUnrecoverableState) {
        await this.notifyClientsAboutUnrecoverableState(appVersion);
      }
      if (err.isCritical) {
  ...
  async notifyClientsAboutUnrecoverableState(appVersion: AppVersion): Promise<void> {
    const broken =
        Array.from(this.versions.entries()).find(([hash, version]) => version === appVersion);
    if (broken === undefined) {
      // This version is no longer in use anyway, so nobody cares.
    ...
    affectedClients.forEach(async clientId => {
      const client = await this.scope.clients.get(clientId);
      client.postMessage({type: 'UNRECOVERABLE_STATE', reason: 'Resource not found.'});
    });
  }
